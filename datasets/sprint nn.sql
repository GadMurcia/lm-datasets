select date_format(
	...
	coalesce(
		(
			...
			from dev_hudi_rwz_cfn.jira_rwz_sprint_issue_report report
...
FROM dev_hudi_rwz_cfn.jira_rwz_sprints sprint
	INNER JOIN dev_hudi_rwz_cfn.jira_rwz_boards board ON board.id = sprint.board_id
	INNER JOIN "dev_hudi_rwz_cfn"."jira_rwz_projects" project ON (board.project_id = project.id)
