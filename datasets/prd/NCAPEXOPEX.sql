...
capacity_teams AS (
	SELECT TRIM(cols [ 2 ]) AS team,
		TRIM(cols [ 3 ]) AS team_id,
		CAST(
			REGEXP_REPLACE(TRIM(cols [ 4 ]), '%', '') AS DOUBLE
		) / 100 AS capacity_development,
		CAST(
			REGEXP_REPLACE(TRIM(cols [ 5 ]), '%', '') AS DOUBLE
		) / 100 AS capacity_support,
		CAST(
			REGEXP_REPLACE(TRIM(cols [ 6 ]), '%', '') AS DOUBLE
		) / 100 AS support_no_prd,
		CAST(
			REGEXP_REPLACE(TRIM(cols [ 7 ]), '%', '') AS DOUBLE
		) / 100 AS support_prd
	FROM parsed_table
),
dbruto1 AS (
	SELECT '|Assigned person|Jira user|LSD resolutor id|Expected monthly sprint swd points|\n|Cristian Salvador Juárez Ramos|Cristian| |32|\n|Isidro Antonio Flores Martínez|Isidro Antonio Flores Martínez| |32|\n|Jose Ernesto Sánchez Sánchez|Jose Ernesto Sanchez| |32|\n|Melissa Gabriela Meléndez Beltrán|Melissa Gabriela Meléndez Beltrán| |32|' AS description,
		'ESBIFLY' as team_id
	UNION ALL
...
members_team AS (
	SELECT team_id,
		TRIM(columnas [ 2 ]) AS assigned_person,
		TRIM(columnas [ 3 ]) AS jira_user,
		TRIM(REGEXP_REPLACE(columnas [ 4 ], '\[', '')) AS lsd_resolutor_id,
		TRIM(
			CASE
...
worked_days as (
	SELECT team_id,
		mes,
		TRIM(columnas [ 2 ]) AS assigned_person,
		COALESCE(TRIM(columnas [ 3 ]), '0') AS worked_days
...
sabana AS (
	...
	FROM prd_hudi_rwz_cfn.jira_rwz_issues i
		JOIN prd_hudi_rwz_cfn.jira_rwz_issuetypes t ON t.id = i.issuetype_id
		JOIN prd_hudi_rwz_cfn.jira_rwz_projects p ON p.key = i.project_key
...
support_points as (
	...
	FROM "prd_hudi_rwz_cfn"."jira_rwz_not_agile_issues"
	WHERE project_key = 'LSD' --AND customfield_12204 LIKE '%@%'
	GROUP BY 1,
		2,
		3
),
