WITH sprints AS (
	SELECT distinct id,
		board_id
	FROM "dev_hudi_rwz_cfn"."jira_rwz_sprints"
),
notCompletedQA AS (
	SELECT m.sprint_id,
		m.board_id,
		COUNT(status_name) AS QANotCompleted
	FROM "dev_hudi_rwz_cfn"."jira_rwz_sprint_issue_report" m
...
FROM "dev_hudi_rwz_cfn"."jira_rwz_sprints" a
	INNER JOIN (
		SELECT sprint_id,
			board_id,
			QANotCompleted
	...
	INNER JOIN "dev_hudi_rwz_cfn"."jira_rwz_boards" b ON (a.board_id = b.id)
	INNER JOIN "dev_hudi_rwz_cfn"."jira_rwz_projects" c on (b.project_id = c.id)